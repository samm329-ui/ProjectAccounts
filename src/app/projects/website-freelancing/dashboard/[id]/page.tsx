...
export default function ProjectDashboard() {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                const selectedClient = clientsData.find((c: any) => c.clientId === clientId);
                setClient(selectedClient);

                const clientPayments = paymentsData.filter((p: any) => p.clientId === clientId);
    ...
    const pieData = [
        { name: 'Paid', value: client.financials.paid, color: '#23D07A' },
        { name: 'Pending', value: client.financials.pending, color: '#FFB86B' }
    ];
    ...
    return (
        ...
        <CardContent>
            <div className=\"text-2xl font-bold text-white\">₹{client.financials.totalValue.toLocaleString()}</div>
                    </CardContent >
                </Card >
        ...
        <CardContent>
            <div className=\"text-2xl font-bold text-white\">₹{client.financials.paid.toLocaleString()}</div>
                    </CardContent >
                </Card >
        ...
        <CardContent>
            <div className=\"text-2xl font-bold text-white\">₹{client.financials.profit.toLocaleString()}</div>
                    </CardContent >
                </Card >
            </div >
